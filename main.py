...
app.layout=dbc.Container([
    ...
    dbc.Row([
      dbc.Col(
        dcc.Input(id='api-public',placeholder="API KEY PUBLIC",type="password"),
      )
    ]
    ),
    dbc.Row(
        [
            dbc.Col(
                dcc.Input(id='api-secret',placeholder="API SECRET KEY",type="password"),
            )
        ]
    ),
    html.Div(
    ...
    dbc.Row([
        dbc.Col(html.Button('Trade', id='trade', n_clicks=0)),
    ]),
    dbc.Row([
...
@app.callback(Output('output-keys','children'),Input('trade','n_clicks'),State('api-public','value'),State('api-secret','value'))
def update_config_variable(trade,api_pub,api_sec):
    if trade:
        url="https://raw.githubusercontent.com/kbenzle/tradr/main/SignalInput.csv"
        file=requests.get(url)
        print(file)
        load_dotenv('config_paramter.env')
        print(os.environ['API_KEY'])
        print(os.environ['API_SECRET'])
        os.environ['API_KEY']=api_pub
        os.environ['API_SECRET'] = api_sec
        print(os.environ['API_KEY'])
        print(os.environ['API_SECRET'])
        print(sys.executable)
        call(["python", "trade_script.py","SignalInput.csv"])
        p=''.join(random.choice('*') for i in range(len(api_pub)))
        s=''.join(random.choice('*') for i in range(len(api_sec)))
        return ["Public Key: "+p," Private Key: "+s]
